...
uint8_t Uart_Receive_Handler(Uart_Instance_t *uart_instance)
{
    ...
    /* 检查UART的空闲中断标志位是否置位 */
    uint8_t ret = __HAL_UART_GET_FLAG(uart_instance->uart_package.uart_handle, UART_FLAG_IDLE) ;
    if(ret != RESET)
    {
        Uart_Rx_Idle_Callback(uart_instance);
    }
    
    // else if(*(uint32_t *)(uart_instance->uart_package.uart_handle) == &huart1)
    // {
    //     Uart_Rx_Idle_Callback(uart_instance);
    // }
    else
...
// /* Yilong 于2025/4/24 注：
//     由于明天就要用了，但是按照正常的框架写法有BUG且迟迟找不到，遂决定在这里临时拉依托，日后修改。
// */
...
//     {
//         Vofa_RxCallback_Fun(AttCom_instance, Size);
//     }
// }
