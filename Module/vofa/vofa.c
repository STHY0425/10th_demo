...
uint8_t VOFA_Get_Data(uint8_t *data,pub_vofa_pid *vofa_pid)
{
    ...
    switch(data[2])
    {
        ...
        case 0xA4:
            ...
            break;
        case 0xA5:
            vofa_pid->i_limit = vofa_data.value;
            break;
    }
    memset(data,0,28);    
    return 1;
...
uint8_t VOFA_SendEND(VOFA_Instance_t *vofa_instance)
{
    ...
    ret = HAL_UART_Transmit(vofa_instance->vofa_uart_instance->uart_package.uart_handle,
                         END_buf,4, 10); 
    if(ret != HAL_OK)
	{
			LOGINFO("vofa send end failed!,%d",ret);
            return 0;
	}
    return 1;
}
...
uint8_t VOFA_SendFloat(VOFA_Instance_t *vofa_instance,float data)
{
    ...
    // 使用 HAL 库发送字节数组
    ret = HAL_UART_Transmit(vofa_instance->vofa_uart_instance->uart_package.uart_handle,
                          buffer, sizeof(float), HAL_MAX_DELAY);
